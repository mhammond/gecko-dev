...
const {Dictionnaire, Enumeration, copieDictionnaire, copieEnumeration, copieEnumerations,
   copieCarte, EnumerationAvecDonnees, switcheroo, Retourneur, DictionnaireNombresSignes, DictionnaireNombres,
   Stringifier, Optionneur, OptionneurDictionnaire} = Rondpoint;
add_task(async function() {
   ...
   Assert.deepEqual(await copieEnumerations([Enumeration.UN, Enumeration.DEUX]), 
   ...
   // Ints
   await affirmAllerRetour([0, 1, -1, -2147483648, 2147483647], rt.identiqueI32.bind(rt));
   ...
   await affirmAllerRetour([0, 1, -1, Number.MAX_SAFE_INTEGER, Number.MIN_SAFE_INTEGER], rt.identiqueI64.bind(rt));
   await affirmAllerRetour([0, Number.MAX_SAFE_INTEGER], rt.identiqueU64.bind(rt));

   // Floats
   const equalFloats = (a, b) => Assert.ok(Math.abs(a - b) <= Number.EPSILON);
   await affirmAllerRetour([0.0, 0.5, 0.25, 1.5], rt.identiqueFloat.bind(rt), equalFloats);
   ...
   await affirmAllerRetour([0.0, 0.5, 0.25, 1.5], rt.identiqueDouble.bind(rt), equalFloats);


   // Strings
   await affirmAllerRetour(["", "abc", "null\u0000byte", "été", "ښي لاس ته لوستلو لوستل", "😻emoji 👨‍👧‍👦multi-emoji, 🇨🇭a flag, a canal, panama"], rt.identiqueString.bind(rt));

   await affirmAllerRetour([-1, 0, 1].map(n => new DictionnaireNombresSignes(n, n, n, n)), rt.identiqueNombresSignes.bind(rt), (a, b) => Assert.deepEqual(a, b));

   await affirmAllerRetour([0, 1].map(n => new DictionnaireNombres(n, n, n, n)), rt.identiqueNombres.bind(rt), (a, b) => Assert.deepEqual(a, b));
   ...
   // Ints
   await affirmEnchaine([0, 1, -1, -2147483648, 2147483647], st.toStringI32.bind(rt));
   await affirmEnchaine([0, 0xFFFFFFFF], st.toStringU32.bind(rt));


   // Longs
   // NOTE: we cannot represent greater than `Number.MAX_SAFE_INTEGER`
   await affirmEnchaine([0, 1, -1, Number.MAX_SAFE_INTEGER, Number.MIN_SAFE_INTEGER], st.toStringI64.bind(rt));
   ...
   await affirmAllerRetour([["a", "b"], []], op.sinonSequence.bind(op), (a, b) => Assert.deepEqual(a, b));
   ...
   // enums
   await affirmAllerRetour([Enumeration.UN, Enumeration.DEUX, Enumeration.TROIS], op.sinonEnum.bind(op));

   // Testing defaulting properties in record types.
   const defaultes = new OptionneurDictionnaire();
   const explicite = new OptionneurDictionnaire(-8, 8, -16, 0x10, -32, 32, -64, 64, 4.0, 8.0, true, "default", [], Enumeration.DEUX, null);

   Assert.deepEqual(defaultes, explicite);

   // …and makes sure they travel across and back the FFI.

   await affirmAllerRetour([defaultes, explicite], rt.identiqueOptionneurDictionnaire.bind(rt), (a, b) => Assert.deepEqual(a, b));
})
